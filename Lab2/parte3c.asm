.data
	...
	guion: .asciiz " - "
	resultado: .asciiz "\nEl resultado de la división potencia/exponente es: "
	punto: .asciiz "."
	resultadoFinal: .asciiz "\n\nLa aproximación para la función logaritmo natural \nen torno a 0, de orden 12, es: "
	resultadoPositivos: .asciiz "\n\nSuma positivos: "
	resultadoNegativos: .asciiz "\n\nSuma negativos: "
	signo: .asciiz "-"
.text
	#397 instrucciones
	#Este programa acepta un número entero hasta 5 puesto que con el número 6, al elevarlo a 12, se sale del
	...
	#Comienza en 3, ya que que la función seno hiperbólico es impar, pero el valor ingresado se suma al final
	addi $t0,$zero,3
	#Se crea una copia de $t0 en $t1, ya que se va modificando
	add $t1,$zero,$t0
	
	#se hace el llamado a la subrutina
	jal logaritmoNatural
	...
	logaritmoNatural:
		...
		li $v0, 4 #codigo 4 para imprimir un string
		la $a0, mensajeResultado #se imprime un mensaje para ingresar el número
		syscall #llamada al sistema
		
		#Se muestra el exponente
		...
		potencia:
			...
			copiar2:
				...
				#para poder hacer la mutiplicación entre lo acumulado en $s2 y el valor ingresado 
				add $s2,$zero,$s4
		...
		#Se divide $s4 (potencia) en $t0 (exponente)
		dividir:
			parteEntera:
				...
				sub $s4,$s4,$t0 #a $s4 se le resta lo de $s3
			...
			decimal1:
				...
				add $s4,$s4,$t0 #a $s4 se le suma lo de $s3
				...
				div1:
					...
					sub $s4,$s4,$t0 #a $s4 se le resta $s3
			...
			decimal2:
				...
				#$t4 corresponde al resto de la división
				#luego se multiplica por otro registro ($t3) que almacena un 10
				add $s4,$s4,$t0 #como $s4 era negativo, se restaura su valor anterior
	...
	logaritmoNatural2:
		...
		potenciaNeg:
			...
			copiar2Neg:
				...
				#$s4 queda en 0, para poder ir guardando el resultado final correctamente
				sub $s4,$s4,$s4
			j  multiplicar2Neg #salta a la subrtuina multiplicar2
		...
		#Se divide $s4 (potencia) en $t0 (factorial)
		dividirNeg:
			parteEnteraNeg:
				#la parte entera se calcula restando lo de s3 a s4, y si $s4 queda negativo
				#se va a calcular el primer decimal
		...
		sumar1Decimal1Neg:
			...
			j restaurarRegistrosNeg #sino salta a restaurar registros
	...
	primeroMenor:
		...
		signoNegativo:
			...
			#Se imprime mensaje para mostrar un signo menos
			li $v0, 4 #codigo 4 para imprimir un string
